...
### 3. Counting how many BedWar maps

**Java**
```java
final int games = AllGamesKt.getALL_GAMES().stream().filter(game -> game instanceof BedWar).map(Game::getMap).collect(Collectors.toSet()).size();
```
**Kotlin**
```kotlin
